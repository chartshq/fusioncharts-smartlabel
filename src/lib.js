var lib = {
	init: function (win) {
		...
		lib = {
			...
			spanAdditionRegx: /(<[^<\>]+?\>)|(&(?:[a-z]+|#[0-9]+);|.)/gi,

			spanAdditionReplacer:
				'$1<span class="' + classNameWithTag + '">$2</span>',

			spanRemovalRegx: new RegExp(
				"\\<span[^\\>]+?" +
					classNameWithTag +
					"[^\\>]{0,}\\>(.*?)\\<\\/span\\>",
				"ig",
			...
			createContainer: function (containerParent) {
				...
				if (
					containerParent &&
					(containerParent.offsetWidth ||
						containerParent.offsetHeight)
				) {
					if (containerParent.appendChild) {
						containerParent.appendChild(
			...
			parseStyle: function (style = {}) {
				...
				parsedStyle.fontSize =
					(style.fontSize || style["font-size"] || "12px") + "";
				parsedStyle.fontVariant =
					style.fontVariant || style["font-variant"] || "normal";
				parsedStyle.fontWeight =
					style.fontWeight || style["font-weight"] || "normal";
				parsedStyle.fontStyle =
					style.fontStyle || style["font-style"] || "normal";
				parsedStyle.fontFamily =
					style.fontFamily || style["font-family"] || "Verdana,sans";

				parsedStyle.fontSize +=
					parsedStyle.fontSize.indexOf("px") === -1 ? "px" : "";
				return parsedStyle;
			},
			/*
